...
<script>
	function updateSort(obj){
		...
		$.post('__URL__/update/navTabId/listrec_article/callbackType/closeCurrent' , {'article_id':$id,'sort':$value} , function(){
			$(obj).html($value);
		});
		navTabPageBreak();
	}
...
<div class="pageHeader">
	<form rel="pagerForm" onsubmit="return navTabSearch(this,'<?php echo ($action); ?>');" method="post">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a  target="selectedTodo" target="dialog" rel="ids[]" href="__URL__/rubAll/navTabId/list<?php echo ($action); ?>" class="delete" ><span>批量放入回收站</span></a></li>
		...
		<tbody>
			<?php if(is_array($list)): foreach($list as $key=>$vo): ?><tr target="item_id" rel="<?php echo ($vo["article_id"]); ?>">
					...
					<td><?php echo ($vo["title"]); ?></td>
										<td >
					<input onblur="updateSort(this);" type="text" style="width:30px;border:0;" name="<?php echo ($vo["article_id"]); ?>" value="<?php echo ($vo["sort"]); ?>" > 
					</td>
